...
public class Lambdas {
    ...
    public List<String> search(List<String> list) {
        Predicate<String> string = s -> s.replaceAll("[^a-zA-Z]", "").length() == 3 && s.startsWith("a");
        return list.stream().filter(string).collect(Collectors.toList());
    }
    ...
    public void threadPrintNL(List<Integer> list, PrintStream out) {
        (new Thread(() -> list.forEach(x -> out.print(x + " ")))).start();
    }
}
